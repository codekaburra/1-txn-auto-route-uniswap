...
contract ZapEstimator is Ownable {
    ...
    /// @notice  estimating output amount of swap _inputAmount of _tokenIn to _tokenOut
    /// @param _fromToken must be a ERC20, input as address_zero as ETH will be consider as same amount of zap.WETH()
    /// @param _toToken must be a ERC20, input as address_zero as ETH will be consider as same amount of zap.WETH()
    ...
    function estimateZapTokenToTokenAmountsOut(
        ...
        address toToken = _toToken == address(0) ? zap.WETH() : _toToken;
        require(zap.isToken(fromToken), "Zap:estimateZapTokenToTokenAmountsOut: invalid fromToken Address");
        require(zap.isToken(toToken), "Zap:estimateZapTokenToTokenAmountsOut: invalid toToken Address");
    ...
    function estimateAddLiquidityOutputAmount(
        ...
        require(zap.isLP(_toLP), "Zap:estimateAddLiquidityOutputAmount: invalid _toLP Address");
    ...
    function estimateZapOutToTokenOutputAmount(
        ...
        address _to = _toToken == address(0) ? zap.WETH() : _toToken;
        {
            if (IUniswapV2Pair(_fromLP).token0() == _to) {
                outputAmount = amount0;
            } else {
                (, uint256[] memory swapAmountsForToken0) = _estimateZapTokenToTokenAmountsOut(
                    IUniswapV2Pair(_fromLP).token0(),
                    _to,
                    amount0
                );
    ...
    function _estimateSwapAmountOut(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(zap.getLiquidityPoolAddress(_tokenIn, _tokenOut))
            .getReserves();
        ...
        return IUniswapV2Router02(zap.ROUTER()).getAmountOut(_inputAmount, reserveA, reserveB);
    }
    ...
    function _estimateZapTokenToTokenAmountsOut(
        ...
        if (_toToken != address(0) && _fromToken != _toToken) {
            ...
            swapAmounts = IUniswapV2Router02(zap.ROUTER()).getAmountsOut(_inputAmount, swapPath);
        } else {
